...
## Sidebar {.sidebar}
```{r}
# Variable selectors for cross-tabulation
selectInput(
  'titanic_var1', 
  'Row Variable:', 
  choices = c('Survived', 'Sex', 'Class'),
  selected = 'Survived'
)
...
output$titanic_table <- renderDT({
  # Create base cross-tabulation
  cross_tab <- table(
    dataset_titanic[[input$titanic_var1]], 
    dataset_titanic[[input$titanic_var2]]
  )
  
  # Convert to desired format based on user selection
  result <- if (input$display_type == 'freq') {
    as.data.frame(cross_tab)
  } else {
    as.data.frame(prop.table(cross_tab, margin = 1))
  }
  ...
  result |>
    ...
    # Format percentages if selected
    mutate(across(
      -1,
      ~if (input$display_type == 'pct') scales::percent(., accuracy = 0.1) else .
    ))
})
